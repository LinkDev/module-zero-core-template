...
export class SampleServiceProxy {
    ...
    protected processGet(response: Response): SampleDto {
        const responseText = response.text();
        const status = response.status; 
     ...
     * @return Success
     */
    getAll(filter?: string, maxResultCount?: number, sorting?: string, skipCount?: number): Observable<PagedResultDtoOfSampleDto> {
        ...
        return this.http.request(url_, options_).map((response) => {
            ...
            if (response instanceof Response) {
                ...
                return <Observable<PagedResultDtoOfSampleDto>><any>Observable.throw(response);
        });
    }

    protected processGetAll(response: Response): PagedResultDtoOfSampleDto {
        const responseText = response.text();
        const status = response.status; 

        if (status === 200) {
            let result200: PagedResultDtoOfSampleDto = null;
            let resultData200 = responseText === "" ? null : JSON.parse(responseText, this.jsonParseReviver);
    ...
    protected processCreate(response: Response): SampleDto {
        const responseText = response.text();
        const status = response.status; 
    ...
    protected processUpdate(response: Response): SampleDto {
        const responseText = response.text();
        const status = response.status; 
...
export class PagedResultDtoOfSampleDto {
    totalCount: number;
    items: SampleDto[];
    ...
    static fromJS(data: any): PagedResultDtoOfSampleDto {
        return new PagedResultDtoOfSampleDto(data);
    }
    ...
    clone() {
        const json = this.toJSON();
        return new PagedResultDtoOfSampleDto(JSON.parse(json));
    }
}
