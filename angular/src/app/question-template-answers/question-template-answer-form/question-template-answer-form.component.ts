import { Component, ViewChild, Injector, Output, EventEmitter, ElementRef, OnInit, AfterViewInit,ViewChildren,QueryList } from '@angular/core';
import { ModalDirective } from 'ngx-bootstrap';
import * as moment from 'moment';
import { QuestionTemplateAnswerServiceProxy, QuestionTemplateAnswerDto, PagedResultDtoOfQuestionTemplateAnswerDto } from '@shared/service-proxies/service-proxies';
import { QuestionTemplateServiceProxy, QuestionTemplateDto, PagedResultDtoOfQuestionTemplateDto } from '@shared/service-proxies/service-proxies';
import { FormComponentBase } from '@shared/form-component-base';
import * as _ from "lodash";

@Component({
  selector: 'question-template-answer-form-modal',
  templateUrl: './question-template-answer-form.component.html'
})
export class QuestionTemplateAnswerFormComponent extends FormComponentBase<QuestionTemplateAnswerDto> implements OnInit, AfterViewInit {
	questionTemplateIdList: QuestionTemplateDto[] = null;
	
    constructor(
        injector: Injector,
			private _questionTemplateAnswerService: QuestionTemplateAnswerServiceProxy, 
		private _questionTemplateService: QuestionTemplateServiceProxy
    ) {
        super(injector);
    }

	ngOnInit() {

    }

	show(id?:string): void {
        if (!id) {
            this.active = true;
            this.modal.show();
            this.item = new QuestionTemplateAnswerDto();
			this.item.id='00000000-0000-0000-0000-000000000000';
			this.item.questionTemplateId= '00000000-0000-0000-0000-000000000000';
            this.isNew = true;
        }
        else {
            this._questionTemplateAnswerService.get(id)
                .subscribe(
                (result) => {
                    this.item = result;
                    this.active = true;
                    this.modal.show();
                });
            this.isNew = false;
        }
    }

	save(): void {
        console.log(this.item);
        this.saving = true;

        if (this.isNew) {
            this._questionTemplateAnswerService.create(this.item)
                .finally(() => { this.saving = false; })
                .subscribe(() => {
                    this.notify.info(this.l('Saved Successfully'));
                    this.close();
                    this.modalSave.emit(null);
                });
        }
        else {
            this._questionTemplateAnswerService.update(this.item)
                .finally(() => { this.saving = false; })
                .subscribe(() => {
                    this.notify.info(this.l('Saved Successfully'));
                    this.close();
                    this.modalSave.emit(null);
                });
        }
    }
}