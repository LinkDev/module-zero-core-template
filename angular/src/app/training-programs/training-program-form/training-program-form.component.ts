...
@Component({
  selector: 'training-program-form-modal',
  templateUrl: './training-program-form.component.html'
})
export class TrainingProgramFormComponent extends FormComponentBase<TrainingProgramDto> implements OnInit, AfterViewInit {
	...
	show(id?:string): void {
        if (!id) {
            this.active = true;
            this.modal.show();
            this.item = new TrainingProgramDto();
            ...
            this._trainingProgramService.get(id)
                .subscribe(
                (result) => {
                    this.item = result;
                    this.active = true;
	...
	save(): void {
        ...
        if (this.isNew) {
            this._trainingProgramService.create(this.item)
            ...
            this._trainingProgramService.update(this.item)
                .finally(() => { this.saving = false; })
                .subscribe(() => {
                    this.notify.info(this.l('Saved Successfully'));
                    this.close();
